...
def pick_variables(df):
    excluded_cols = [model_class_col_name, probability_col_name]
    # Label y-label & fair_features
    describe("Choose a y-label")
    ylabel = st.selectbox("", set(df.columns).difference([model_class_col_name, probability_col_name]))

    # Choose fair features:
    describe("Choose features on which to evaluate model fairness")
    fair_features = st.multiselect("", set(df.columns).difference([model_class_col_name, probability_col_name, ylabel]))
...
def pick_xaxis_color_plt1(fair_features):
    ...
    color_col = st.selectbox("(This should be Categorical Data)", set(fair_features).difference([xlabel]))
